...
**MCP 표면에는 쓰기가 없다.** 주문 제출·설정 변경·삭제 오퍼레이션이 카탈로그에 존재하지 않으므로,
MCP 로 붙은 에이전트가 만들 수 있는 최악은 "거절 기록"이지 원장 변경이 아니다.

**CLI 는 다르다.** 카탈로그 밖에 상태를 바꾸는 것이 둘 있고 CLI 에서 도달 가능하다 —
`paper open`(모의 원장 genesis)과 `--strategy-module`(임의 파일 실행). 둘 다 **사람의 손이
필요한 명령**이고, 에이전트는 대신 실행하지 않는다. 아래 "상태를 바꾸기 전 안전 체크리스트" 참조.
...
2. **거절은 두 층이고 다음 행동이 다르다.**
   *호출 거절*(`isError: true` / exit 1·2)은 **이 호출이 그대로는 답이 안 된다**는 뜻이다 —
   입력을 고치거나 **환경**을 고쳐야 한다. 어느 쪽인지는 `error` 값이 말한다(아래 에러표).
   그대로 재시도하는 것은 어느 쪽에도 답이 아니다.
...
1. 시리즈 파일이 있는지 확인한다. **이 표면에는 시세 조회 오퍼레이션이 없다** — 캔들 파일은
   사용자가 준다. 없으면 요청하라(지어내지 마라). 다만 "이 제품은 시세를 못 가져온다" 고 말하지
   마라 — 어댑터는 있고 이 표면에 배선되지 않았을 뿐이다(아래 "없는 것 — 두 종류").
...
`isError: true` 는 **"이 호출은 그대로는 답을 낼 수 없다"** 는 뜻이지 항상 당신 잘못이라는 뜻이
아니다. 앞의 넷은 **호출**을 고쳐야 하고, 뒤의 둘은 **환경**을 고쳐야 한다. 이유를 안 보고
그대로 재시도하면 여섯 경우 모두 같은 답이 돌아온다.

| `error` | 무엇이 잘못됐나 | 당신이 할 일 |
|---|---|---|
| `unknown_operation` | 호출 | `list_operations` 를 먼저 부르고 이름을 고쳐라 |
| `invalid_input` | 호출 | `describe_operation` 으로 스키마를 받아 입력을 고쳐라 |
| `unknown_strategy` | 호출 | `strategy.list` 로 이름 확인 |
| `invalid_params` | 호출 | 파라미터를 고쳐라. 교차 필드 규칙(`fast < slow`)은 스키마에 안 나온다 |
| `configuration_required` | 환경 | **설정하고 다시 불러라** — 기다려도 안 된다. `DATABASE_URL` 을 주고 서버 재시작 |
| `unavailable` | 환경 | **저장소 읽기가 실패했다.** 원인은 이 표면 밖에서 알 수 없다 — 드라이버 장애일 수도, 이 저장소의 버그일 수도 있다(연결 문자열이 샐 수 있어 원문을 옮기지 않는다). 재시도는 **한두 번까지**, 반복되면 사람에게 |
...
**당신이 부를 수 있는 명령 중** 돈을 만드는 것은 하나뿐이다: `paper open`. 그래서 **카탈로그에
없다** — 즉 MCP 로는 도달할 수 없다. 하지만 **CLI 에는 있다.** "에이전트 표면에 쓰기가 없다"가
CLI 까지 참인 문장이라고 읽지 마라.

(엔진 자체는 주문 제출·취소·erasure 를 전부 할 수 있다 — `PaperTradingService` 의 seam 이다.
그것들이 **명령으로 노출되지 않았을 뿐**이고, 그 차이는 아래 "없는 것 — 두 종류" 에 적혀 있다.)
...
3. **시리즈 파일 경로** — 사용자가 준 경로만 읽어라. 경로를 추측해 파일 시스템을 뒤지지 마라.
   `--series` 는 **JSON 이 아닌 파일도 열어본다** — 잘못된 경로를 주면 실패하지만, 애초에 시리즈가
   아닌 파일을 겨누는 것은 이 플래그의 용도가 아니다. 파일 내용은 실패 메시지로 새지 않는다.
   **크기 상한은 없다** — bar 수는 무제한이라 아주 큰 파일은 프로세스 메모리를 그만큼 쓴다
   (실측: 50만 bar ≈ +100MB, `--dry-run` 도 동일). 출처를 모르는 거대한 시리즈 파일은
   돌리기 전에 bar 수를 사용자에게 확인하라.

---

## 사람에게 제어를 돌려줘야 하는 경우

- `error.code: api` / exit 2 가 났다 (저장소 읽기 실패 — 재시도로 풀리지 않고, 원인은 이 표면에서
  단정할 수 없다. "DB 가 죽었다" 고 보고하지 말고 "저장소에 닿지 못했다" 고 보고하라)
- `configuration_required` 를 받았고 환경 변수를 설정해야 한다
- `paper open` 이 필요하다 (돈 원장 생성)
- `--strategy-module` 이 필요해 보인다 (플래그를 켜는 것은 사람의 결정)
- 사용자가 **실거래**를 기대하고 있다 — 라이브 주문 경로는 코드가 throw 로 막는다. 모의뿐이다.
- 시세·공시 데이터를 가져와 달라고 한다 — **이 표면에** 그 오퍼레이션이 없다. "제품에 없다" 가
  아니라 "여기서는 못 부른다, 캔들 파일을 달라" 로 답하라.
- 모의 **주문을 넣어** 달라고 한다 — 엔진에는 주문 seam 이 있지만 에이전트 표면에 열려 있지 않다
  (티켓 40 confirm token 선행). 백테스트로 대신할 수 있는지 확인하고, 아니면 사람에게 넘겨라.
...
사용자에게 "안 됩니다" 라고 답하기 전에 **어느 쪽인지** 확인하라. 둘은 사용자에게 전혀 다른 말이다.

### ① 이 표면에 배선되지 않은 것 (제품에는 있다)

**"이 제품은 그걸 못 한다" 고 말하지 마라 — 거짓이다.** 정확히는 *이 CLI·MCP 표면에 오퍼레이션이
없다*. 사용자가 원하면 다른 경로(웹 앱)나 다음 슬라이스를 안내하라.

| 기능 | 저장소의 실체 | 왜 카탈로그에 없나 |
|---|---|---|
| 시세 조회 | `financial-information/data/` — KIS·scripted·국채·ECB FX 어댑터 | `market-server.ts` 가 `server-only` + Next alias 라 CLI/MCP 가 소비하면 seam 규칙을 건드린다. Stage 3 가 이 층을 재작성하므로 그 뒤 별도 슬라이스 |
| 공시 조회 | `dart-filings-information.ts` + `publicFilingsServer()` | 위와 같음 |
| 차트/캔들 생성 | `financial-information/chart/` (`buildChartSeries` 등) | 위와 같음. 그래서 **지금은** 캔들 파일을 사용자가 줘야 한다 |
| 모의 주문 제출 | `PaperTradingService.prepare()` → `change()` — **실재하는 production seam** 이고 백테스트 러너가 바로 이걸 쓴다 | write 는 티켓 40 의 confirm token 이 선행돼야 에이전트 표면에 오른다(T11). 엔진에 없는 게 아니라 **의도적으로 안 열었다** |
| 설정 변경·데이터 삭제 | erasure 경로 존재 | 위와 같은 write 정책 |

### ② 이 제품에 구조적으로 없는 것

- **실거래(라이브) 주문.** 이것만은 "없다" 가 문자 그대로 참이다 — 코드가
  `runtime-policy.ts` 에서 `"Live Trading submission is unavailable in this release"` 로
  **throw 한다.** 표면 문제가 아니라 불변식이다. 사용자가 실거래를 기대하면 즉시 바로잡아라.
- **뉴스 조회.** 계약 주석에 종류로만 언급되고 어댑터가 없다. (README 옛 서술에 남아 있으나
  피벗 배너가 붙은 재작성 대상 문단이다.)

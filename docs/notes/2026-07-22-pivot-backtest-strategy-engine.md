...
### 해외주식 시세 — **가능. 확정** (2026-07-26 조사, KIS 공식 저장소 `examples_llm/overseas_stock` 직독)

기존 §4는 해외를 "모의 주문은 일부 종목+지정가만"이라는 **주문** 제약만 알고 있었다. **시세**는 미조사였고,
이번에 1차 출처로 확정했다. 결론: **해외 백테스트는 성립하며, 실전 키가 필요 없다.**

| 용도 | 경로 (`/uapi/overseas-price/v1/quotations/…`) | TR | 실전/모의 |
|---|---|---|---|
| 현재가 | `price` | `HHDFS00000300` | **공통** (`env_dv` real\|demo) |
| 기간별시세 (일·주·월) | `dailyprice` | `HHDFS76240000` | **공통** |
| 종목·지수·환율 기간별 (일·주·월·년) | `inquire-daily-chartprice` | `FHKST03030100` | 지원 |
| 실시간 지연체결가 (WS) | — | `HDFSCNT0` | — |

- **거래소 코드**: `NAS` 나스닥 · `NYS` 뉴욕 · `AMS` 아멕스 · `TSE` 도쿄 · `HKS` 홍콩 ·
  `SHS`/`SZS` 상해·심천 · `SHI`/`SZI` 상해·심천지수 · `HSX`/`HNX` 호치민·하노이
- **실시간성 (결정적)** — `delayed_ccnl.py` docstring 원문: **"미국 - 실시간무료(0분지연) /
  홍콩, 베트남, 중국, 일본 - 15분지연"**. 아시아 실시간은 HTS 유료 신청.
  → 미국은 `freshness: realtime` 을 정직하게 붙일 수 있고 아시아는 `delayed` — 둘 다 기존 모델이 지원한다.

**착수 시 걸릴 것 3개 (실측):**
1. **엔드포인트가 유니버스로 갈린다** — `inquire-daily-chartprice` 는 `FID_COND_MRKT_DIV_CODE`
   (N=해외지수·X=환율·I=국채·S=귀금속선물)를 주지만 **미국 개별주는 Dow30·Nasdaq100·S&P500 만**
   조회된다. 그 밖 종목은 `dailyprice`. 어댑터가 두 경로를 갖게 된다.
2. **수정주가 플래그 `modp`** — `dailyprice` 파라미터. T8 S3 게이트에서 `priceBasis: total_return`
   을 fail-closed 로 거부하게 만들어놨으므로(adjusted close 를 raw 로 체결 = 무고지 왜곡),
   "modp 를 뭘로 두고 어떤 priceBasis 로 라벨하나"가 해외 어댑터의 첫 설계 결정이다.
3. **세금은 오히려 유리** — 미국주는 증권거래세 없음(SEC fee 는 매도대금 ~0.00278%, 양도소득세는
   체결 회계 밖). 우리 모델의 `taxClass` **미선언 = 무세**가 사실상 정확하다. 설계문서 §3 비범위의
   "미국장 SEC fee — 대상 시장 미정"은 이로써 판정 가능해졌다.

**판정**: 해외는 "불가능"이 아니라 **"미착수"**다. 단 v1 범위에는 넣지 않는다 — 휴장일 캘린더
(미국 것 없음)·세션 판정·거래소 코드·수정주가 정책이 전부 신규 표면이고 T10(CLI/MCP)·Stage 3 가
먼저다. **v1 국내, 해외 v2** (§9-2 참조).
...
- **해외 `dailyprice` 1콜당 최대 행수·과거 데이터 깊이** — 공식 샘플이 재귀 페이지네이션
  (`max_depth=10`)만 명시하고 건수를 안 적었다. 실호출로만 확정된다 (국내 기간별시세는 100건/call)
...
이후 — 엔진   ← 다음 세션은 여기부터 (2026-07-24 Stage 2-c 완료로 이동. Stage 3 는 CLI 골격 뒤)
  ...
  T10. 전략 정의 층 + CLI + MCP  (선행 티켓 초안 있음: .scratch/financial-terminal/issues/40·41)
    · 에이전트 인터페이스 설계 메모 (2026-07-22, HyeokjaeLee/koreainvestment-cli 전체 정독 결과):
      ...
      ← 정정 (2026-07-26 T10 S4 실행 시): ①②의 전제 둘이 실제와 갈렸다.
        ① **"주문 전 안전 체크리스트"는 쓸 수 없었다** — 티켓 40(confirm token)을 T11 로 이월하면서
           이 표면에 주문 명령 자체가 없다. 없는 명령의 체크리스트를 지어내면 그것이야말로 이 저장소가
           금지하는 "그럴듯한 거짓"이라, 실재하는 상태 변경 셋으로 각색했다 — `paper open`(돈 genesis),
           `--strategy-module`(사실상 RCE, 플래그 뒤), 시리즈 파일 경로. 주문 체크리스트는 T11 몫.
        ② **installation.md 는 별도 파일로 만들지 않고 README 안에 두었다.** 설치 경로가 3단계뿐이고
           `docs/release/setup.md` 와 겹치는데, Stage 3 가 패키징(`package-release.ts`·manifest)을
           재정의하며 두 문서를 모두 다시 쓴다 — 지금 가르면 그때 합칠 것이 하나 늘 뿐이다.
           "WebFetch 말고 curl" 에 해당하는 디테일은 이 저장소 사정에 맞게 치환했다: 원격 fetch 대신
           클론한 워킹 트리에서 SKILL.md 를 직접 읽을 것, 그리고 **`npm run` 으로 기동하지 말 것**
           (npm 이 배너를 stdout 에 찍어 `--json` 파이프와 MCP 의 JSON-RPC 스트림을 깨뜨린다 — 실측).
        ③ 왜 이 쪽이 나았나: 문서가 코드에 대해 참인 문장만 담게 됐고, 그 참을 `tests/t10-skill-doc.test.ts`
           가 상시 단언한다(오퍼레이션 이름·거절 이유 전 멤버·안전한 기동 형태).
        ④ 어긋난 것: 없음 — 작업량은 예상대로였다. 다만 별도 installation.md 를 기대하고 오는 다음
           세션은 README §"에이전트로 쓰기" 를 보면 된다.
...
2. **대상 시장** — 국내주식만 / 국내 + 지수 / 해외 포함.
   **← 근거 확보 (2026-07-26)**: 해외 시세 API 실측으로 "가능하되 v1 비범위" 판정 — §4 "해외주식 시세" 참조.
   국내(주식+지수)는 이미 실측·배선됨. 남은 결정은 **해외를 v2 언제 여느냐**로 좁혀졌다
...
4. **README 재작성** — 이름이 `provenance`가 됐으니 상단 문구를 새로 쓸 것.
   ...
   - **← 정정 (2026-07-26 README 재작성 실행 시). 이 항목의 후반부는 폐기됐다.**
     ① **틀린 전제**: "같은 엔진"을 리드 문구의 두 기둥 중 하나로 권했다.
     ② **실제**: 2026-07-24 경쟁 대조에서 **기각**됐다 — backtest+live 동일 엔진은
        NautilusTrader·QuantConnect Lean 의 **간판 기능**이라 차별점이 아니고, 포트폴리오로
        내세우면 면접에서 정확히 그 지점이 걸린다. 리드는 **빈 교차점**(한국 시장 규칙 ×
        에이전트 인터페이스(MCP) × 데이터 정직성)으로 썼고, 같은 엔진은 차별점이 아니라
        *사실 서술* 한 줄로 내렸다. Lean·Nautilus 는 prior art 로 명시 인용한다.
     ③ **왜 이 쪽이 나았나**: 검증 가능한 주장만 남았다. 데이터 정직성은 `backtest run` 실출력
        블록(`winRate: unavailable/no_sells`)이 곧 증거이고, 한국 규칙은 거래세·휴장일 코드가
        정본이다. 반면 "같은 엔진"은 선행 도구가 이미 하는 말이라 아무것도 증명하지 못한다.
     ④ **어긋난 것**: 없음 — 전반부("값을 모르면 지어내지 않는다")는 그대로 리드가 됐다.
       다만 §9-4 만 읽고 시작하면 폐기된 축을 리드로 쓰게 되므로 이 정정이 필요했다.
     ⑤ **함께 적은 것**: 체결 모델 서술 주의 — Lean "동일"이 아니라 **선형 재파라미터화**다
       (10%·5/20/25bps vs Zipline/Lean 2.5%·quadratic). "동일 모델" 주장 금지, §4 정정 참조.

...
> ⚠️ **2026-07-22 방향 전환 진행 중** — 제품 범위를 "백테스트 + 실시간 모의투자 엔진, 얼굴은 CLI/MCP"로 좁히고
> 웹·인증 트랙은 단계적으로 걷어낸다(A안 확정, 게스트 터미널 존치만 F11 결정). 작업 시작 전
> **[방향 전환 메모(2026-07-22)](docs/notes/2026-07-22-pivot-backtest-strategy-engine.md)** 를 먼저 읽을 것 —
> 확정 결정·조사로 확인된 외부 사실(재조사 금지)·폐기된 안(재논의 금지)·실행 순서(Stage 0~3 + T8~T12)가 있다.
> 다음 작업은 **Stage 1**(공통 죽은 코드 삭제)이다. 아래 용어 정의 중 Actual Portfolio 관련 항목은 T4에서 축소 예정이다.

...
describe("provision (genesis exactly-once)", () => {
  it("seeds cash once and treats redelivery as a duplicate", () => {
    const book = new BrokerPaperBook();
    const acct = account();
    expect(book.provision(WORKSPACE, acct, [{ amount: 100_000, currency: "USD" }]).status).toBe("applied");
    expect(book.provision(WORKSPACE, acct, [{ amount: 100_000, currency: "USD" }]).status).toBe("duplicate");
...
describe("local submit + derived reservation CAS (§9)", () => {
  it("commits a pending_submission order whose reservation is derived before any broker ack", () => {
    ...
    const outcome = book.submitLocal(WORKSPACE, acct, clientOrder("co-1"), limitBuy(5, 110));
  ...
  it("refuses an unaffordable buy with no order row (overspend 0)", () => {
    const { book, acct } = seeded();
    const outcome = book.submitLocal(WORKSPACE, acct, clientOrder("co-big"), limitBuy(1_000, 110));
  ...
  it("refuses an oversell with no position (oversell 0)", () => {
    const { book, acct } = seeded();
    const outcome = book.submitLocal(WORKSPACE, acct, clientOrder("co-sell"), limitSell(5, 120));
  ...
  it("refuses a market order fail-closed (no pre-submit price bound in the broker lane)", () => {
    ...
    const outcome = book.submitLocal(WORKSPACE, acct, clientOrder("co-mkt"), payload);
  ...
  it("treats a redelivered identical local submit as a duplicate, not a second order", () => {
    const { book, acct } = seeded();
    book.submitLocal(WORKSPACE, acct, clientOrder("co-1"), limitBuy(5, 110));
    const replay = book.submitLocal(WORKSPACE, acct, clientOrder("co-1"), limitBuy(5, 110));
...
describe("external event fold — durable unique + quarantine (§9/AT-08)", () => {
  it("folds accepted → fill into acknowledged/filled with exact cash literals", () => {
    ...
    book.submitLocal(WORKSPACE, acct, co, limitBuy(5, 110));

    const ack = book.ingest(WORKSPACE, acct, event(co, { kind: "accepted", externalIdentity: "E1", revision: 1, body: { externalOrder: "X9" } }));
    ...
    const fill = book.ingest(WORKSPACE, acct, event(co, { kind: "fill", externalIdentity: "F1", revision: 2, body: { quantity: 5, price: { amount: 109.9, currency: "USD" } } }));
  ...
  it("keeps the remaining reservation exact across a partial fill", () => {
    ...
    book.submitLocal(WORKSPACE, acct, co, limitBuy(25, 110));
    book.ingest(WORKSPACE, acct, event(co, { kind: "accepted", externalIdentity: "E1", revision: 1, body: { externalOrder: "X1" } }));
    book.ingest(WORKSPACE, acct, event(co, { kind: "fill", externalIdentity: "F1", revision: 2, body: { quantity: 10, price: { amount: 109.9, currency: "USD" } } }));
    const order = book.state(WORKSPACE, acct).orders[0]!;
  ...
  it("re-applies nothing on stream/poll duplicate delivery of the same event identity+revision", () => {
    ...
    const co = clientOrder("co-1");
    book.submitLocal(WORKSPACE, acct, co, limitBuy(5, 110));
    book.ingest(WORKSPACE, acct, event(co, { kind: "accepted", externalIdentity: "E1", revision: 1, body: { externalOrder: "X9" } }));
    const fill = event(co, { kind: "fill", externalIdentity: "F1", revision: 2, body: { quantity: 5, price: { amount: 109.9, currency: "USD" } } });
    book.ingest(WORKSPACE, acct, fill);
    const replay = book.ingest(WORKSPACE, acct, fill);
    expect(replay.status).toBe("duplicate");
  ...
  it("quarantines a same-revision divergent payload without touching the book", () => {
    ...
    book.submitLocal(WORKSPACE, acct, co, limitBuy(5, 110));
    book.ingest(WORKSPACE, acct, event(co, { kind: "accepted", externalIdentity: "E1", revision: 1, body: { externalOrder: "X9" } }));
    book.ingest(WORKSPACE, acct, event(co, { kind: "fill", externalIdentity: "F1", revision: 2, body: { quantity: 5, price: { amount: 109.9, currency: "USD" } } }));
    const divergent = book.ingest(WORKSPACE, acct, event(co, { kind: "fill", externalIdentity: "F1", revision: 2, body: { quantity: 5, price: { amount: 109.8, currency: "USD" } } }));
  ...
  it("releases the whole reservation on a broker rejected event", () => {
    ...
    const co = clientOrder("co-1");
    book.submitLocal(WORKSPACE, acct, co, limitBuy(5, 110));
    book.ingest(WORKSPACE, acct, event(co, { kind: "rejected", externalIdentity: "R1", revision: 1, body: {} }));
    const order = book.state(WORKSPACE, acct).orders[0]!;
  ...
  it("refuses an over-limit buy fill at the boundary (provider degradation, no money movement)", () => {
    ...
    book.submitLocal(WORKSPACE, acct, co, limitBuy(5, 110));
    book.ingest(WORKSPACE, acct, event(co, { kind: "accepted", externalIdentity: "E1", revision: 1, body: { externalOrder: "X9" } }));
    const over = book.ingest(WORKSPACE, acct, event(co, { kind: "fill", externalIdentity: "F1", revision: 2, body: { quantity: 5, price: { amount: 110.01, currency: "USD" } } }));
...
describe("cancellation axis + late fill (§9 three axes)", () => {
  it("releases remaining reservation on cancel_confirmed and still applies a late pre-cancel fill exactly once", () => {
    ...
    book.submitLocal(WORKSPACE, acct, co, limitBuy(10, 110));
    book.ingest(WORKSPACE, acct, event(co, { kind: "accepted", externalIdentity: "E1", revision: 1, body: { externalOrder: "X1" } }));
    book.ingest(WORKSPACE, acct, event(co, { kind: "fill", externalIdentity: "F1", revision: 2, body: { quantity: 4, price: { amount: 109.9, currency: "USD" } } }));
    book.ingest(WORKSPACE, acct, event(co, { kind: "cancel_confirmed", externalIdentity: "C1", revision: 4, body: {} }));
    ...
    expect(book.ingest(WORKSPACE, acct, late).status).toBe("applied");
    expect(book.ingest(WORKSPACE, acct, late).status).toBe("duplicate");
  ...
  it("refuses a fill whose revision is at or after the confirmed cancellation", () => {
    ...
    book.submitLocal(WORKSPACE, acct, co, limitBuy(10, 110));
    book.ingest(WORKSPACE, acct, event(co, { kind: "accepted", externalIdentity: "E1", revision: 1, body: { externalOrder: "X1" } }));
    book.ingest(WORKSPACE, acct, event(co, { kind: "cancel_confirmed", externalIdentity: "C1", revision: 2, body: {} }));
    const post = book.ingest(WORKSPACE, acct, event(co, { kind: "fill", externalIdentity: "F9", revision: 3, body: { quantity: 1, price: { amount: 109.9, currency: "USD" } } }));
  ...
  it("keeps the reservation intact on cancel_rejected and never regresses a confirmed cancellation", () => {
    ...
    book.submitLocal(WORKSPACE, acct, co, limitBuy(5, 110));
    book.ingest(WORKSPACE, acct, event(co, { kind: "accepted", externalIdentity: "E1", revision: 1, body: { externalOrder: "X1" } }));
    book.ingest(WORKSPACE, acct, event(co, { kind: "cancel_rejected", externalIdentity: "C1", revision: 2, body: {} }));
    expect(book.state(WORKSPACE, acct).orders[0]!.cancellation).toBe("rejected");
    expect(usd(book, acct).reserved).toBe(550);

    book.ingest(WORKSPACE, acct, event(co, { kind: "cancel_confirmed", externalIdentity: "C2", revision: 3, body: {} }));
    const regress = book.ingest(WORKSPACE, acct, event(co, { kind: "cancel_rejected", externalIdentity: "C3", revision: 4, body: {} }));
...
describe("sell path", () => {
  it("reserves shares for a sell and credits exact proceeds on fill", () => {
    ...
    book.submitLocal(WORKSPACE, acct, buy, limitBuy(5, 110));
    book.ingest(WORKSPACE, acct, event(buy, { kind: "accepted", externalIdentity: "E1", revision: 1, body: { externalOrder: "X1" } }));
    book.ingest(WORKSPACE, acct, event(buy, { kind: "fill", externalIdentity: "F1", revision: 2, body: { quantity: 5, price: { amount: 109.9, currency: "USD" } } }));

    const sell = clientOrder("co-s");
    expect(book.submitLocal(WORKSPACE, acct, sell, limitSell(5, 120)).status).toBe("accepted");
    // All 5 shares now back a sell; a second oversell must fail CAS.
    expect(book.submitLocal(WORKSPACE, acct, clientOrder("co-s2"), limitSell(1, 120))).toEqual({ status: "refused", reason: "insufficient_position" });

    book.ingest(WORKSPACE, acct, event(sell, { kind: "accepted", externalIdentity: "E2", revision: 1, body: { externalOrder: "X2" } }));
    book.ingest(WORKSPACE, acct, event(sell, { kind: "fill", externalIdentity: "F2", revision: 2, body: { quantity: 5, price: { amount: 120.5, currency: "USD" } } }));
...
describe("erasure fence (SEC-09 substrate)", () => {
  it("suppresses provision and ingest at or below the fence", () => {
    ...
    const late = book.ingest(WORKSPACE, acct, event(co, { kind: "accepted", externalIdentity: "E1", revision: 1, body: { externalOrder: "X9" } }));

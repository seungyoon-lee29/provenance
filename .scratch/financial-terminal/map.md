...
- [23 - Persistence seam (P1)](./issues/23-persist-identity-erasure-spine.md) **open / ready-for-agent** (현 frontier): gate-2를 실제로 성립시키는 선행 티켓. Unit-of-work 트랜잭션 포트 + 스키마 컨벤션(append-only·revision·idempotency·fence-first) 확정 + Identity·PersonalCache end-to-end 이관 + 스택 backup/restore/deletion 드릴. 돈 원장·outbox·event(F6~F10)는 P2+ 클러스터별 최상위 gate로 분리 — gate-2 완전 종결은 거기서. 사용자 스코프 결정 2026-07-19.

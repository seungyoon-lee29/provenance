...
> 여기서 완료로 기록한 상태 보유 ActualPortfolio · `/f6-portfolio` 은(는) 2026-07-22 피벗의 Stage 1/2 컷에서 **삭제됐다.**
...
- [x] **B5 presentation + perf + closeout** — `/f6-portfolio` synthetic 표면(dev-only·force-dynamic·network-off): 실제 journal(supersede 교정 포함)+service+completeness 표현을 scripted port 3종으로 구동 — complete(합계+비중), partial(알려진 소계+누락 목록·**total 요소 부재**), unavailable(합계/소계 0개·원통화 값 유지), aggregate lot은 기준일과 함께 **이름으로** 표시(WS-06/UF-06). browser spec 10 green(desktop+mobile). perf: initial projection 450/800ms p95(500-entry journal) vitest green. check 858/66 green. commit 813bdfb. → **ticket 15 closeout**.

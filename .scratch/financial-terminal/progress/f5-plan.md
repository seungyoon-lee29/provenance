...
- [~] **B2 잔여**
  ...
  - [ ] **미완**: Delivery Authorization Context 4종 resolve(발급) + occurrence-engine records/occurrences 이관(B2.5b).
- [ ] **B3 Delivery Fact append-only + Alert Channel Availability** — Fact 상태(queued/accepted/delayed/delivered/bounced/complained/suppressed/seen/failed/expired) append-only·승격 0(FencedStore). Availability 5-way 합성.

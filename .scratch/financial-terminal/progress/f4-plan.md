...
> 여기서 완료로 기록한 research-assistant · 데이터 패널 라우트 은(는) 2026-07-22 피벗의 Stage 1/2 컷에서 **삭제됐다.**
...
- [~] **B5 presentation/browser (라우트+배선 완료, Playwright 스펙 위임 중)**
  - [x] **presenter view-model 층**: `data-panel-presenter.ts`가 F1-proven `presentGuestPanel`(guest 실브라우저 렌더 머신)를 재사용해 market/evidence/research 값을 렌더. **AT-01 DOM 불변식(값은 available에만·stale은 값+degradation notice·unavailable/failed는 값 없음)을 B1 catalog 정본으로 25 test 검증** = bullet 1의 "API AND DOM" 중 DOM view-model 절반.
  - [x] **실 page/route mount + withDeadline 실배선**: `src/app/f4-panels/page.tsx`(dev/test 전용, prod 404). 16-case 시장 매트릭스 + 6 evidence + 2 research를 `withDeadline`(DATA 10s/AI 20s) 래핑 read로 로드, 재사용 `GuestPanel`로 렌더. `toMarketPanelState/toEvidencePanelState/toResearchPanelState` DOM-mount seam 추가. hang read에 400ms 데모 deadline → timeout이 DOM에 뜸(무한 spinner 없음, bullet 6). **스모크 검증(full runtime env): primary-value 7 = available 7, failed 10 + unavailable 8, 값은 available에만.** typecheck·lint clean.
  - [~] **Playwright 리터럴 DOM 스펙 + explicit-unavailable screenshot**: Sonnet 서브에이전트에 위임(작성·디버깅 루프, 저비용). `tests/browser/f4-panels.spec.ts`가 실브라우저에서 AT-01 매트릭스(25 패널, 7=7 불변식, deadline timeout, presenter 라벨)를 단언. 산출 검증 후 gate.

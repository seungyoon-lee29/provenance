...
3. `Identity`
   - presentation용 interface는 `resolve(sessionProof)`, `requestAccountEmail(command, clientProof)`와 `consumeAccountChallenge(command: { kind: "link" | "manual_code"; proof: string }, clientProof)`다. request command는 이메일 확인·복구·로그인 purpose와 request idempotency를 받고 enumeration-safe outcome을 반환한다. 새 account의 이메일 확인과 아직 verified address가 없는 복구에는 PendingAccountEmailTarget, 검증된 활성 계정의 로그인·복구에는 WorkspaceSecurityEmailEndpoint를 내부에서 선택하며 선택 결과를 응답으로 노출하지 않는다. 존재하지 않거나 eligible하지 않은 sign-in 대상은 같은 응답을 반환하되 target, action material과 외부 Intent를 만들지 않는다. consume command는 두 challenge 종류 모두 같은 family row·attempt counter를 사용해 CSRF-protected POST에서 compare-and-consume와 session issuance를 한 transaction으로 처리한다.
   - 비로그인 방문자는 즉시 guest 문맥을 받고, 로그인 사용자의 User Workspace 접근은 검증된 세션에서만 부여한다.
   - requestAccountEmail은 최소화된 Account Security Event, purpose에 맞는 `PendingAccountEmailTarget | WorkspaceSecurityEmailEndpoint`, hash-only verifier와 별도의 envelope-encrypted `DeliveryActionMaterialReference<AccountChallengeMaterial>`을 같은 transaction에 저장하고 durable internal event를 낸다. NotificationCenter의 server-only `AccountSecurityDelivery.plan(eventReference)`가 이를 exactly-once Delivery Intent로 투영하고 account-security renderer만 server-only resolver로 action material을 해석한다. provider 수락·확정 실패·expiry 또는 accept-unknown reconciliation 뒤 material을 삭제하며 다른 module은 원문 주소·token/code 또는 session issuer에 접근하지 않는다.
   ...
   - 지속형 financial email worker에는 transient session이나 직렬화한 Viewer Context가 아니라 server-only `resolveFinancialDelivery(reference)`가 workspace membership, financial-consent epoch와 verified-address revision을 재검사해 FinancialEmailDeliveryContext를 발급한다. `resolveAccountChallengeDelivery(eventReference, targetReference)`는 transient Viewer Context 없이 target variant를 해석해 `pending | workspace` AccountChallengeDeliveryContext를 발급한다. pending variant는 pending identity, purpose, request-security epoch, target/action-material expiry와 deletion fence를, workspace variant는 검증된 기존 계정의 로그인·복구 purpose/expiry, account authorization epoch, workspace membership/account active, verified-address revision과 deletion fence를 재검사한다. `resolveSecurityNoticeDelivery(eventReference, endpointReference)`는 별도로 allowlisted authenticated security notice purpose/expiry, workspace membership/account active, security-notice epoch, verified-address revision과 deletion fence를 재검사해 SecurityNoticeDeliveryContext를 발급한다. 세 context는 보통 logout만으로 폐기되지 않지만 목적을 서로 바꿀 수 없고 financial context는 consent/address/membership 변경, account/security context는 해당 epoch, address revision, account state 변경과 administrative deletion에 폐기된다.
...
5. `NotificationCenter`
   ...
   - Delivery Intent의 `sourceReference?`와 `deliveryActionMaterialReference?`는 독립 필드다. financial email은 `AlertOccurrence + channel=email + source + UnsubscribeMaterial + WorkspaceFinancialEmailEndpoint`, financial Web Push는 `AlertOccurrence + channel=web_push + source + WorkspaceWebPushEndpoint`만 허용한다. account challenge는 `AccountSecurityEvent + channel=email + no source + AccountChallengeMaterial`에 purpose가 허용하는 `PendingAccountEmailTarget | WorkspaceSecurityEmailEndpoint`를 조합한다. allowlisted `authenticated_security_notice`는 `AccountSecurityEvent + channel=email + WorkspaceSecurityEmailEndpoint`에 한해 두 reference 없이 허용하고 pending/financial/push endpoint·다른 purpose는 거절한다. renderer는 provider 수락 전 retry에서 같은 material/payload hash를 재사용하고 accepted·확정 실패·expiry 또는 accept-unknown reconciliation 뒤 원문 material만 삭제한다.
   - 외부 adapter는 Delivery Intent가 durable commit된 뒤 worker에서만 호출한다. financial email은 workspace membership·financial consent epoch·verified-address revision·License Scope·expiry, Web Push는 device binding auth epoch·consent·License Scope·expiry, pending account email은 Account Security Event purpose·request security epoch·target expiry, WorkspaceSecurityEmailEndpoint account challenge는 purpose/expiry·account authorization epoch·workspace/account state·verified-address revision·deletion fence, authenticated security notice는 purpose/expiry·workspace/account state·security-notice epoch·verified-address revision·deletion fence를 dispatch 직전에 다시 검사한다. transient login session epoch를 지속형 financial email consent나 account/security delivery epoch로 사용하지 않는다.
6. `TerminalView`

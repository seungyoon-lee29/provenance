...
DeliveryActionMaterialVault는 tagged variant별 AAD를 강제한다. AccountChallengeMaterial은 Account Security Event·purpose·expiry와 `pending identity + request security epoch` 또는 `workspace + security endpoint + account authorization epoch`에, UnsubscribeMaterial은 workspace·endpoint·topic·consent lineage·Delivery Intent에 묶인다. 허용된 template renderer 외 resolve는 거절하고 raw token/code는 log·outbox·metadata에 없어야 한다. provider accepted, definite failure, challenge/Intent expiry와 accept-unknown reconciliation 뒤 purge 시점을 고정 clock으로 검증하며 purge 전 retry는 최초 payload hash와 정확히 같은 material만 해석한다.
...
- Notification delivery contract는 financial alert의 template render와 dispatch 직전에 source-owned reference의 external-delivery License Scope를 다시 해석한다. 회수 race에서는 제한 원문이 payload·subject·log에 0이고 generic metadata-only 또는 `suppressed/license_restricted`여야 한다. Delivery Intent의 `sourceReference?`와 `deliveryActionMaterialReference?`를 독립 필드로 두고 property test한다. financial email은 `AlertOccurrence + email + source + UnsubscribeMaterial + WorkspaceFinancialEmailEndpoint`, financial Web Push는 `AlertOccurrence + web_push + source + WorkspaceWebPushEndpoint`, account challenge email은 `AccountSecurityEvent + email + no source + AccountChallengeMaterial`에 purpose가 허용하는 `PendingAccountEmailTarget | WorkspaceSecurityEmailEndpoint`, allowlisted `authenticated_security_notice`는 `AccountSecurityEvent + email + WorkspaceSecurityEmailEndpoint + no source + no action material` 조합만 허용한다. authenticated notice 성공 fixture와 함께 wrong cause/channel/reference/action/target, 비허용 purpose와 그 밖의 조합은 renderer/provider 호출 0이어야 한다. Financial endpoint는 membership·financial-consent epoch·verified-address revision, Security endpoint는 membership/account-state·security-notice epoch·verified-address revision, Web-Push endpoint는 workspace/device-binding auth epoch·consent, PendingAccountEmailTarget은 Account Security Event·pending identity·purpose·request security epoch·expiry에 묶인다. cause/target 조합을 property test해 pending target을 financial/Web Push/authenticated notice에 쓰거나 security endpoint challenge를 anonymous pending identity와 조합하면 호출 0이며 keyed destination fingerprint 외 raw target은 outbox·webhook artifact에 없어야 한다.
- 목적별 authorization race를 분리해 검증한다. delivery resolver에는 transient Viewer Context를 전달하지 않는다. logout 뒤에도 membership·consent epoch·verified-address revision이 유효한 financial email은 전달되고 consent/address 변경·membership 종료 뒤에는 억제된다. PendingAccountEmailTarget challenge는 AccountChallengeDeliveryContext의 pending variant가 pending identity·purpose·request-security epoch·target/action-material expiry·deletion fence를, WorkspaceSecurityEmailEndpoint challenge는 workspace variant가 purpose/expiry·account authorization epoch·account state·address revision을 확인한다. authenticated security notice는 별도 SecurityNoticeDeliveryContext의 purpose/expiry·account state·security-notice epoch·address revision이 유효하면 financial opt-out과 보통 logout 뒤에도 전달된다. context와 variant는 purpose를 서로 바꿀 수 없고 address 변경, endpoint disable, 해당 stale epoch, account 종료와 deletion fence 뒤에는 억제된다. cross-workspace endpoint swap, `channel=web_push`, financial/security endpoint 교환은 renderer/provider 호출 0이다. Web Push는 device binding auth epoch/logout policy를 사용하며 다른 context의 조건을 요구하지 않는다.
...
- security/account email suite는 주소 HMAC, device/session, IP-prefix와 미검증 익명·proof-verified recovery·authenticated notice의 분리된 전역 budget, 분산 공격 및 enumeration-safe 응답을 검증한다. 공격자가 untrusted budget을 소진해도 proof를 통과한 정상 recovery와 allowlisted 로그인/credential 변경 challenge·authenticated notice가 protected budget, `channel=email`과 purpose에 맞는 target으로 성공해야 한다. 같은 notice를 PendingAccountEmailTarget, WorkspaceFinancialEmailEndpoint, WorkspaceWebPushEndpoint, 다른 workspace endpoint 또는 비허용 purpose로 계획하면 외부 호출이 0이어야 한다. magic-link token은 purpose와 `pending identity + request security epoch | workspace + security endpoint + account authorization epoch`에, unsubscribe token은 workspace·endpoint·topic·channel·consent lineage에 묶고 모두 256-bit 이상·hash-only이며 cross-scope replay를 거절한다. active consent lineage의 오래된 unsubscribe token은 현재 opt-in을 해지하고, opt-out 뒤 반복 POST는 일반 성공과 기존 상태를 유지하며, 명시적 re-opt-in의 새 lineage는 이전 token에 영향받지 않아야 한다. magic-link GET/prefetch는 소비하지 않고 CSRF-protected 동시 POST 중 한 개만 session을 만든다. portfolio alert의 source-owned reference와 reference 없는 account event Notification Record도 조회·보존·삭제 fixture에 포함한다. no-store/no-referrer/no-app-token-log와 Resend Free external content processing 고지를 검증한다. provider open/click과 notification click은 `seen`을 만들지 않고 foreground Viewer Context acknowledgement만 만든다.
...
- `Identity.consumeAccountChallenge`의 manual code는 별도 hash-only 10-character Crockford Base32 value이며 link와 같은 purpose, `pending identity + request security epoch | workspace + security endpoint + account authorization epoch`와 10분 expiry를 가져야 한다. family 5회 실패, link/code kind의 동시 public command와 성공 뒤 replay를 fault-inject해 session 최대 하나, 어느 한 경로 성공·lockout·expiry 뒤 두 credential 모두 무효, raw code/log 0을 검증한다.

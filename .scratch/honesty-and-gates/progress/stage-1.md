...
## 라운드 6 — OF-6 (분할이 예약을 다시 쓴다)

**Blast radius**: `src/modules/paper-trading/internal/journal.ts` (fold + 저널 경계 판정),
`service.ts` (주문 페이로드 검증). 돈 경로이며 저장 포맷은 무변경.
**검증 tier**: **top** (collaboration.md 의 tier 표 기준 — money 경로 + 공개 거부 이유 추가).

### 설계가 한 번 죽었다

착수 전 grilling 으로 받은 결정은 "예약 가치를 정확히 보존할 수 없으면 그 분할을 거부한다"
였다. 구현·검증까지 끝낸 뒤 **적대 라운드 1 의 F-1 이 그것을 죽였다**: 정확 보존은 홀수 센트
단가를 전부 거부한다 — 2:1 의 **50%**, 3:1 의 67%, 4:1 의 75%. 100주 @ $101.03 같은 평범한
주문 하나가 분할 자체를 막는다.

**진짜 결함은 코드가 아니라 고지였다.** 착수 전 사용자에게 알린 잔여 위험은 "1센트 지정가
주문이 있는 계정"이었다. 실제 범위는 정직한 분할의 절반이다. 두 자릿수 틀린 전제 위에서
받은 승인이었고, **산수 한 줄이면 착수 전에 알 수 있었다.** OF-1 의 A안에서 밟은 것과 같은
형태다 — 그때는 "A 로는 테스트가 안 초록이 된다"를 늦게 알았다.

교체한 규칙과 근거는 `open-findings.md` 의 OF-6 이 정본이다.

### 라운드 2 가 라운드 1 의 수정에서 High 2건을 찾았다

한 라운드로 끝내지 말라는 규칙이 또 값을 냈다.

| # | 요지 | 상태 |
| --- | --- | --- |
| F-1 | 한 주문의 두 가격(지정가·예약 단가)을 따로 양자화한 것이 결함. 죽은 홀드도, **모자란 홀드도** 생긴다 | 접수, 수정 승인됨 (미구현) |
| F-2 | `Math.ceil` 은 하한 1 을 만든다. 수량에 상한이 없어 `10,000,000:1` 한 방이 홀드를 잔고 전액으로 | 접수, 수정 승인됨 (미구현) |
| F-3 | `reserved ≤ balance` 는 계정 불변식이 아니다 — **HEAD 에서 같은 수치로 재현**되는 기존 결함 | 새 OF 로 등록 예정 |
| F-4 | 잔고 헤드룸 0~50 minor 면 여전히 50% 거절 | OF-11 에 보강 예정 |
| F-5 | `isExactMinor(gross)` 뮤테이션 생존 — 공허한 가드 | 삭제 예정 |

### 판별력을 어떻게 쟀나

서브에이전트의 "green" 을 믿지 않는다는 규칙대로, blind 저자의 16 통과를 메인이 재현한 뒤
**구현 쪽에 뮤턴트를 넣어** 다시 쟀다. 천장을 분할 대상 주문으로만 좁히니 "천장은 계정
전체다" 1건만 정확히 red — 그 테스트가 실제로 계정 전체 합산을 재고 있다는 증거다.

blind 계약이 v1 → v2 로 바뀌었을 때 **blind 저자를 다시 돌렸다.** v1 을 인코딩한 테스트
5개가 red 였고 그건 옳은 red 다. 메인이 그 테스트를 고쳤다면 축이 사라졌을 것이다.

### 라운드 6 게이트 상태

| 축 | 상태 |
| --- | --- |
| oracle | **완료** — 963 passed / 0 failed, negative-control 46/46 |
| 적대 리뷰 | **2라운드 완료** — v1 3건(High 2), v2 5건(High 2). 리포트 2개 |
| blind 저자 | **2라운드 완료** — 계약 v2 전 조항 충족, 16 통과 |
| 미해소 | **v2 의 F-1·F-2·F-5 는 이 커밋에 안 들어갔다.** 수정 방향은 사용자 승인됨 |

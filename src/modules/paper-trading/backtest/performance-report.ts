 ...
 * (realized P&L per sell) is out of this slice (S4b). If a backtest ever carries
...
function demo(): void {
  ...
  const flat = buildPerformance({
    ...
    seedValue: 1_000_000, finalValue: 1_000_000, equity: [1_000_000], participations: [],
  });
  ...
  const nanSeed = buildPerformance({ currency: "KRW", ...year, seedValue: NaN, finalValue: 100, equity: [100], participations: [] });
  assert(nanSeed.timeWeightedReturn.status === "unavailable" && nanSeed.moneyWeightedReturn.status === "unavailable", "NaN seed → unavailable");
  const tiny = buildPerformance({ currency: "KRW", from: "2024-01-01T00:00:00.000Z", to: "2024-01-01T00:01:00.000Z", seedValue: 100, finalValue: 101, equity: [100, 101], participations: [] });
  ...
  const wipeout = buildPerformance({ currency: "KRW", ...year, seedValue: 100, finalValue: 0, equity: [100, 0], participations: [] });
